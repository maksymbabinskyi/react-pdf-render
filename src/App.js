...
function App() {
  ...
  return (
    <div className="App">
      <div className='input'>
        <textarea placeholder='Change content' onChange={(e) => setContent(e.target.value)}></textarea>
        <button
          onClick={() => setRenderContent(true)}
        >
          Render
        </button>
      </div>
      <PdfViewer content={content} renderContent={renderContent} setRenderContent={setRenderContent} />
    </div>
  );
}
